...
std::string read_file(const std::string & path)
{
  ...
  if (!stream.is_open()) {
    throw std::runtime_error("failed to open M4 ranking report: " + path);
  }
...
std::vector<RankingRow> load_ranking_rows(const std::string & path)
{
  ...
  for (const auto & object : extract_row_objects(json)) {
    RankingRow row;
    row.scenario_id = extract_string(object, "scenario_id");
    row.failure_mode = extract_string(object, "failure_mode");
    ...
    if (!row.scenario_id.empty() && !row.ranker.empty() && row.rank > 0) {
      rows.push_back(row);
    }
  }
  return rows;
...
void set_row_color(Marker & marker, const RankingRow & row)
{
  ...
  const double rank_alpha = std::max(0.22, 0.72 - 0.16 * static_cast<double>(row.rank - 1));
  marker.color.a = rank_alpha;
  if (failure_aware) {
    marker.color.r = row.rank == 1 ? 0.05 : 0.95;
    marker.color.g = row.rank == 1 ? 0.95 : 0.72;
    marker.color.b = 0.12;
    return;
  }
  marker.color.r = 0.14;
  marker.color.g = row.rank == 1 ? 0.45 : 0.65;
  marker.color.b = 1.0;
}
  ...
  double marker_thickness_m)
{
  ...
  marker.header.frame_id = frame_id;
  marker.header.stamp = stamp;
  marker.ns = failure_aware ? "m4_failure_aware_proxy" : "m4_silhouette_only_proxy";
  marker.id = id;
...
std::string compact_shape_name(const std::string & shape_type)
{
  ...
  double depth_m)
{
  ...
  marker.header.frame_id = frame_id;
  marker.header.stamp = stamp;
  marker.ns = "m4_ranking_panel_text";
  marker.id = id;
  marker.action = Marker::ADD;
  marker.type = Marker::TEXT_VIEW_FACING;
  marker.pose.position.x = 0.0;
  marker.pose.position.y = -0.22;
  marker.pose.position.z = depth_m - 0.025;
  marker.pose.orientation.w = 1.0;
  ...
  for (const auto & row : rows) {
    if (row.ranker != "silhouette_only") {
      continue;
    }
    label << row.rank << ":" << compact_shape_name(row.shape_type)
          << "=" << row.total_score << "\n";
  }
...
class M4OfflineRankingMarkersNode : public rclcpp::Node
{
public:
  explicit M4OfflineRankingMarkersNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : rclcpp::Node("m4_offline_ranking_markers_node", options)
  {
    ranking_report_path_ = declare_parameter<std::string>(
      "ranking_report_path", "reports/m4_offline_ranking.json");
    scenario_id_ = declare_parameter<std::string>("scenario_id", "S6");
    marker_topic_ = declare_parameter<std::string>("marker_topic", "/ghost_mgg/hypothesis_markers");
    frame_id_ = declare_parameter<std::string>("frame_id", "d435_depth_optical_frame");
    top_k_ = declare_parameter<int>("top_k", 3);
    ...
    marker_thickness_m_ = declare_parameter<double>("marker_thickness_m", 0.012);
...
private:
  void publish_markers()
  {
    try {
      ...
      for (const auto & row : rows) {
        if (row.scenario_id != scenario_id_ || row.rank > top_k_) {
          continue;
        }
      ...
      if (!selected_rows.empty()) {
        markers.markers.push_back(
          make_summary_text_marker(selected_rows, scenario_id_, frame_id_, stamp, id++, marker_depth_m_));
      }
      marker_pub_->publish(markers);
      if (!reported_success_) {
        RCLCPP_INFO(
          get_logger(),
          "Published %d M4 offline hypothesis marker rows for scenario %s from %s",
          published,
          scenario_id_.c_str(),
      ...
      RCLCPP_WARN_THROTTLE(
        ...
        "Cannot publish M4 offline ranking markers yet: %s", error.what());
    }
  }
  ...
  double marker_thickness_m_ = 0.012;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<M4OfflineRankingMarkersNode>());
  rclcpp::shutdown();
  return 0;
}
